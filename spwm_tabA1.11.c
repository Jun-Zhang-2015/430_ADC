...
//		state &= 0xfffe;									//  从上半周期开始		这两句一起用	
			...
			hsamples = samples<<1;
			qsamples = hsamples>>1;																								//  一个周期的抽样数，为4的倍数；
			samples = hsamples <<1 ;

			idx = hsamples*((float)idx/i); 									//  取老idx计算相位，恢复新相位
			for ( i=0;i<=qsamples;i++)
				{
				sin_tab[i] = sin(i*PI/2/qsamples)*curr_ccr;		//   重新计算计算sin表 考虑了幅度因子
				}
			cflag=0;
			//  
    }
