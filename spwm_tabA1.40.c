...
extern float sinTab(int i);
extern void init_sinTab(int samples);							


#define  abs(x)					((x)>0? x : -(x))
#define  PI  						3.1415926535897932384626
 		
#define  MIN_RATE    		0.5														  //  可修改
#define  MAX_RATE    		50														//  可修改

#define  MIN_SAMPLES		100														//  50Hz波形时一周期抽样256次
#define  Multi_N				100														// 	(MAX_RATE/MIN_RATE)			最大抽样倍数   基数为STD_CCR  
...
__interrupt void TIMERA0_ISR0(void) //Flag cleared automatically
{
   ...
   // c1 = (curr_ccr>>1) - r*curr_ccr*(ma*(float)sin_tab[n]/STD_CCR);				//	必须用常数？	//  ！！！重要： 算出来的TA0CCR1不可能会小于 1 ， 这是通过ma 最大值取值上来控制
    c1 = (STD_CCR>>1)- sinTab(idx)*ma*STD_CCR/2;
