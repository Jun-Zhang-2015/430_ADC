    ...
    while(1)
    {
     	ADC_startConversion(ADC_BASE, ADC_REPEATED_SINGLECHANNEL);
			...
			unsigned int v = 0;
		 	if ( adcvalue==v ) 
		 		{
		 			__delay_cycles(80000/5);				//	延时100ms	 ;Delay for 80000*(1/MCLK)=0.5s 
			...
			//  动了
 			do{
				v=adcvalue;
     		ADC_startConversion(ADC_BASE, ADC_REPEATED_SINGLECHANNEL);
				__delay_cycles(80000/10);
				if (adcvalue< min_adc )
        	min_adc = adcvalue;
      	if (adcvalue>max_adc)
        	max_adc = adcvalue;
			}while(adcvalue!=v);							//  有变，是连续在转，不管，继续读ADC
      ...
      step = step >Multi_N ? Multi_N : step;

			qsamples =  (MAX_SAMPLES/step)>>2;						//  1/4个周期的抽样数
			hsamples = qsamples>1;
			samples = hsamples<<1;												//  一个周期的抽样数，为4的倍数；
			for ( i=0;i<=qsamples;i++)
				{
				sin_tab[i]=sin(i*PI/2/qsamples)*STD_CCR;		//   计算sin表
				}
			cflag--;
			//  
    }
